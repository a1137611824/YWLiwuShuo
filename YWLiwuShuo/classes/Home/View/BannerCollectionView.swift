...
extension BannerCollectionView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        let tabBarViewController = UIApplication.sharedApplication().keyWindow?.rootViewController as! YWBaseTabBarController
        let navigationController = tabBarViewController.viewControllers![tabBarViewController.selectedIndex] as! YWBaseNavViewController
        navigationController.pushViewController(YWCommonBaseViewController(), animated: true)
    }
...
extension BannerCollectionView: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        pageControl.center.x = contentOffsetX + (SCREEN_WIDTH * 0.5)
